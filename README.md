...
In addition a manual for using the GUI is available at [Simulated Rada Waveform Generator GUI manual](docs/Simulated_Radar_Waveform_Generator_GUI_manual.pdf). The manual contains a gloassoray of the paramters that are used for generation. In addition, it includes the exact paramters that were used to generate the refrence RF dataset. For more information see the additional references below. 
